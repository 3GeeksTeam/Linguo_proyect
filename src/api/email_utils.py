...
def generar_enlace_verificacion(email, serializer, salt='email-confirm-salt'):
    ...
    return url_for('api.verificar_email', token=token, _external=True)
...
def enviar_correo_verificacion(email, mail, serializer):
    ...
    link = f"{base_url}/api/verificar/{token}"# Construye el enlace manualmente

    msg = Message('Verifica tu cuenta',
                  recipients=[email],
                  html=f'<p>Haz clic en el siguiente enlace para verificar tu cuenta:</p><a href="{link}">{link}</a>')
    mail.send(msg)
